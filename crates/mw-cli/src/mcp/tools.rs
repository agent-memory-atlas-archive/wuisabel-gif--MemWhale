//! MCP tool implementations shared by stdio and HTTP transports.
...
fn remember_tool(text: &str, client_name: Option<&str>) -> Result<String, String> {
    let id = crate::remember_as(text, None, "agent", client_name, None)?;
    Ok(format!(
...
fn similar_failures_report(conn: &Connection, error_text: &str, command: Option<&str>) -> String {
    ...
    if let Some(cmd) = command {
        if let Some(fp) = crate::error_fingerprint(cmd, error_text) {
            if let Ok(insight) = crate::error_insight(conn, &fp) {
...
mod tests {
    ...
    fn similar_failures_reports_counts_and_pointer() {
        ...
        let fp = crate::error_fingerprint("cargo", err).unwrap();
